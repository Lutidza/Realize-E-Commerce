/**
 * @file src/app/(realize)/[locale]/companies/[[...segments]]/page.tsx
 * @version 0.2.0 – 2025-12-27 13:30
 * @description Публичный список компаний для локализованных маршрутов с ЧПУ.
 */
...
export default async function LocalizedCompaniesPage({ params, searchParams }: PageProps) {
  ...
  if (!parseListResult.ok) {
    ...
    const allowedIdsForCard = resolvedProfile
      ...
      : undefined
    const cityAlias = resolveCompanyCityAlias(company.city, resolvedLocale)
    ...
    const currentPath =
      segments.length > 0
        ? formatLocalePath(resolvedLocale, `/companies/${segments.join('/')}`)
        : formatLocalePath(resolvedLocale, '/companies')
  ...
  const canonicalPath = buildCompanyPath({ ...match, locale: resolvedLocale })
  const currentPath =
    pathSegments.length > 0
      ? formatLocalePath(resolvedLocale, `/companies/${pathSegments.join('/')}`)
      : formatLocalePath(resolvedLocale, '/companies')
